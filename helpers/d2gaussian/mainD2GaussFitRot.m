...
%  non
%% ---------User Input---------------------
MdataSize = 100; % Size of nxn data matrix
mgrid = -MdataSize/2:MdataSize/2-1;
mgrid = mgrid + 0.5;
mmin = min(mgrid);
mmax = max(mgrid);
...
xdata(:,:,1) = X;
xdata(:,:,2) = Y;
[Xhr,Yhr] = meshgrid(linspace(mmin,mmax,300)); % generate high res grid for plot
...
if FitForOrientation == 0
    ...
    x =x(1:5);
    lb = [0,mmin,0,mmin,0];
    ub = [realmax('double'),mmax,(mmax)^2,mmax,(mmax)^2];
...
axis([mmin-0.5 mmax+0.5 mmin-0.5 mmax+0.5 -noise noise+x(1)])
alpha(0.2)  
hold off
...
text(mmin*0.9,+mmax*1.15,string1,'Color','red')
text(mmin*0.9,+mmax*1.2,string2,'Color','red')
text(mmin*0.9,+mmax*1.25,string3,'Color','red')
