...
function psychsr_prepare_stimuli()
    ...
    psychsr_set('stimuli','sine_gratings',1);
%% grating    
    if max(strcmp(data.stimuli.stim_type,'grating')) || max(strcmp(data.stimuli.stim_type,'rand_grating')) || max(strcmp(data.stimuli.stim_type,'grating3')) || max(strcmp(data.stimuli.stim_type,'rot_grating')) ...
            || max(strcmp(data.stimuli.stim_type,'grating_patch')) || max(strcmp(data.stimuli.stim_type,'grating_annulus'))
        bg_color_offset = [0.5, 0.5, 0.5, 1];
        contrast_multiplier = 0.5;
        radius = []; % full screen grating

        if data.stimuli.sine_gratings
            data.stimuli.grating_tex = CreateProceduralSineGrating(win,w,h,...
                bg_color_offset,radius,contrast_multiplier);
        else
            data.stimuli.grating_tex = CreateProceduralSquareGrating(win,w,h,...
                bg_color_offset,radius,contrast_multiplier);
        end
    end
...
%% movie/image  
    if max(strcmp(data.stimuli.stim_type,'movie')) || max(strcmp(data.stimuli.stim_type,'image')) ...
            || max(strcmp(data.stimuli.stim_type,'grating2')) || max(strcmp(data.stimuli.stim_type,'movie_patch'))
        ...
        for i = 1:length(files)
            if strcmp(files{i}(end-3:end),'.mat')
                ...
                mov = 250.*mat2gray(mov); %255/max(max(max(mov)))*mov;
...
%                 
            ...
            if data.stimuli.crop_movie == 1
                ...
                dstRect = [0,0,w,h];
			elseif data.stimuli.crop_movie == 0
			...
			elseif data.stimuli.crop_movie == 2 % rajeevs method
% 				if ratio > wid/height % screen wider than movie, add gray on sides
                    dw = (w-wid*h/height)/2;
                    dstRect = [dw,0,w,h];
%                 else
...
%% patch/annulus
% if max(cellfun(@isempty,strfind(data.stimuli.stim_type,'patch'))) || max(cellfun(@isempty,strfind(data.stimuli.stim_type,'annulus')))
% 	
% end
%% alternating checkboard
    if max(strcmp(data.stimuli.stim_type,'checker'))
        v = data.stimuli.checker_v;
        h = data.stimuli.checker_h;
        
        checker = data.screen.black*ones(h,v);
        d = -floor((h-1)/2)*2:2:floor((v-1)/2)*2;
        B = data.screen.white*ones(max(v,h),length(d));
        checker = full(spdiags(B,d,checker));        
        checker = imresize(checker,[data.screen.height_pixels,data.screen.width_pixels],'nearest');
               
        data.stimuli.checker_tex{1}=Screen('MakeTexture', win, checker);
        data.stimuli.checker_tex{2}=Screen('MakeTexture', win, data.screen.white-checker);        
    end
end