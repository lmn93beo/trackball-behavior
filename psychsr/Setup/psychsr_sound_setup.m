function psychsr_sound_setup()
    ...
    tone_time_short = psychsr_set('sound','tone_time_short',0.1);
    ...
    short_lowcue_RH = [tone_amp*sin(1000*2*pi*(0:1/Fs:tone_time_short-1/Fs)), blank]; 
    short_lowcue_RH = [short_lowcue_RH; short_lowcue_RH];
    ...
    lowleft = lowcue_RH; lowleft(2,:) = 0;
    highright = highcue_RH; highright(1,:) = 0;
%     
    % chirps
    ...
    try
        ...
        % multiple chirps (n = 4)        
        n = 4;        
        t2 = (0:Fs*tone_time/(n+1)-1)/Fs;
        
        mchirp_up = repmat([tone_amp*chirp(t2,f1,T/(n+1),f2) zeros(1,round(Fs*T/n/(n+1)))],2,n);
        mchirp_dn = repmat([tone_amp*chirp(t2,f2,T/(n+1),f1) zeros(1,round(Fs*T/n/(n+1)))],2,n);
%         
%         mchirp_up = repmat([repmat(tone_amp*chirp(t2,f1,T/n,f2),1,4), blank],2,1);
%         mchirp_dn = repmat([repmat(tone_amp*chirp(t2,f2,T/n,f1),1,4), blank],2,1);
	...
	try 
        ...
        data.sound.buffers(30) = PsychPortAudio('CreateBuffer', pahandle, short_lowcue_RH);
        if chirp_flag
            ...
            data.sound.buffers(26) = PsychPortAudio('CreateBuffer', pahandle, mchirp_up);
            data.sound.buffers(27) = PsychPortAudio('CreateBuffer', pahandle, mchirp_dn);
        end
        data.sound.buffers(25) = PsychPortAudio('CreateBuffer', pahandle, vhighcue);
        data.sound.buffers(28) = PsychPortAudio('CreateBuffer', pahandle, lowleft);
        data.sound.buffers(29) = PsychPortAudio('CreateBuffer', pahandle, highright);
