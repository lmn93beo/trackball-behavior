function data = psychsr_train_lickdiscrim3()
% Trial structure:
...
%% variable parameters
    ...
    ntarget = 1800; % # of target-only trials
    
%     response.abort_window = 0.5; % window before stimulus
    response.abort_grace = 1; % grace period after tone
    response.abort_sound = 1;    
    sound.noise_amp = 0.05;        
    sound.noise_time = 3;
    response.abort_timeout = 8;
    response.abort_punish = 1;
...
%% stimuli
    ...
    num_loops = total_duration/(iti+min(off_dur));
    ...
    for i = 1:num_loops                
        ...
        duration(k) = rand*(max(off_dur)-min(off_dur))+min(off_dur);
        stim_type{k} = 'blank';
        k = k+1;
...
function new_loop = feedback_fn(loop)
    ...
    if loop.response  % if lick
        ...
        elseif strcmp(data.stimuli.stim_type{loop.stim+1},'grating') && ...            
                (loop.prev_flip_time > data.stimuli.end_time(loop.stim-1) + data.response.abort_grace)            
            loop.stim = loop.stim - 1;
            time_shift = data.response.abort_timeout;
            data.stimuli.end_time(loop.stim:end)=data.stimuli.end_time(loop.stim:end)+time_shift;
            psychsr_sound(data.response.abort_sound);
            if data.response.abort_punish
