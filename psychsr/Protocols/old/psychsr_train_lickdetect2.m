function data = psychsr_train_lickdetect2()
% Trial structure:
% t=0	tone (0.5s), blank (2+ sec)
%           licking after t=1 prolongs off period for 1s
...
function new_loop = feedback_fn(loop)
    ...
    % prolong off period unless licking stopped for 1 second
...
%         
        if strcmp(data.stimuli.stim_type{loop.stim+1},'grating') 
            if loop.prev_flip_time-max(data.response.licks) < 2
                time_shift = 1;%-loop.prev_flip_time+max(data.response.licks);
                data.stimuli.end_time(loop.stim:end)=data.stimuli.end_time(loop.stim:end)+time_shift;
                fprintf('LONGER OFF %1.3f\n',time_shift);
            end
        end
