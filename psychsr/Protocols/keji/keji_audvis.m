...
sounds = [26, 27, 27, 26, 0, 0, 26, 27]; %[1 13 14 25 21 23 22 24];

contrasts = [1, 1, 1, 1, 1, 1, 0, 0];
frameRate = 1;
...
% rect_template{1}=[0 0 1 1]; % fullscreen
 rect_template{1}=[0.05 0.1 0.4 0.9]; % left
 rect_template{2}=[0.5 0.1 0.85 0.9]; % right
 rect_template{3}=[0.05 0.1 0.4 0.9]; % left
 rect_template{4}=[0.5 0.1 0.85 0.9]; % right
 rect_template{5}=[0.05 0.1 0.4 0.9]; % left
 rect_template{6}=[0.5 0.1 0.85 0.9]; % right
 rect_template{7}=[0.05 0.1 0.4 0.9]; % left
 rect_template{8}=[0.5 0.1 0.85 0.9]; % right
...
total_duration = repeats*length(sounds)*(offTime+onTime);
...
%% stimuli
num_stimuli = repeats*length(sounds)*2;
...
for i = 1:num_stimuli
	...
	elseif mod(i-1,2)==1
		...
		rect{i} = rect_template{order(i/2)};
	end
end


stimuli = psychsr_zip(num_stimuli,total_duration,stim_type,duration,contrast,orientation,spat_freq,temp_freq,rect,randnumframes,sound_id, checker_v, checker_h);
stimuli.blackblank = 0;

%% input parameters into psychsr
params = psychsr_zip(screen,response,card,sound,stimuli,presentation);

data = params; %#ok<NASGU>
