...
% *** 02/03/2016 HS ***
% *** 03/14/2016 HS *** ***************************************************
    data.response.aborted = []; % record aborted trial.
% *** 03/14/2016 HS *** ***************************************************
% *** 03/29/2016 HS *** ***************************************************
    data.response.helped = []; % record helped trial.
% *** 03/29/2016 HS *** ***************************************************
% *** 05/20/2016 HS *** ***************************************************
    data.response.helped_time = []; % record helped trial time.
% *** 05/20/2016 HS *** ***************************************************
% *** 08/23/2016 HS *** ***************************************************
% Add trial number to plot in performance fig.
    data.response.n_hits_trlN  = [];
    data.response.n_false_trlN = [];
% *** 08/23/2016 HS *** ***************************************************
end
...
%     str = [str, sprintf('FALSE%% %3d%%   %3d%% of %d\n',per_false,round(mean(data.response.n_false)*100),length(data.response.n_false))];
    str = [str, sprintf('HIT%%   %3d%%   %3d%% (%d/%d)\n',per_hit,round(  mean(data.response.n_hits )*100),sum(data.response.n_hits ),length(data.response.n_hits ))];
    str = [str, sprintf('FALSE%% %3d%%   %3d%% (%d/%d)\n',per_false,round(mean(data.response.n_false)*100),sum(data.response.n_false),length(data.response.n_false))];
% *** 08/25/2016 HS *** ***************************************************
% *** 01/14/2016 HS ***
...
%         str = [str, sprintf('FALSE2%%       %3d%% of %d\n',f2,length(ind))];
        str = [str, sprintf('FALSE2%%       %3d%% (%d/%d)\n',f2,sum(ind),length(ind))];
% *** 08/25/2016 HS *** ***************************************************
    end
...
%     str = [str, sprintf('DELAY%% %3d%%   %3d%% of %d\n',per_delay,round(mean(data.response.n_delay)*100),length(data.response.n_delay))];
    str = [str, sprintf('DELAY%% %3d%%   %3d%% (%d/%d)\n',per_delay,round(mean(data.response.n_delay)*100),sum(data.response.n_delay),length(data.response.n_delay))];
% *** 08/25/2016 HS *** ***************************************************
...
%         end
        data.response.per_hits10  (tmpTrl) = per_hit;
        data.response.per_false10 (tmpTrl) = per_false;
        data.response.per_dprime10(tmpTrl) = dprime10;
% *** 08/25/2016 HS *** ***************************************************
% *** 02/03/2016 HS ***
% *** 08/25/2016 HS *** ***************************************************
...
%             end
%         end
        data.response.per_hits  (tmpTrl) = mn_n_hits *100;
        data.response.per_false (tmpTrl) = mn_n_false*100;
        data.response.per_dprime(tmpTrl) = dprime;
        if data.response.p_targ_after_cr > 0 || data.response.p_ntarg_after_fa > 0
            data.response.per_false2 (tmpTrl) = mn_n_false2*100;
            data.response.per_dprime2(tmpTrl) = dprime2;
        end
% *** 08/25/2016 HS *** ***************************************************
% *** 08/23/2016 HS *** ***************************************************
% Prepare dots plots for Hits/Miss/FA/CR.
        % trial numbers
        tmpX_H = data.response.n_hits_trlN (logical( data.response.n_hits ));
        tmpX_M = data.response.n_hits_trlN (logical(~data.response.n_hits ));
        tmpX_F = data.response.n_false_trlN(logical( data.response.n_false));
        tmpX_C = data.response.n_false_trlN(logical(~data.response.n_false));
        % percentage for Hits/False Alarms
        tmpD_H = data.response.per_hits10 (tmpX_H);
        tmpD_M = data.response.per_hits10 (tmpX_M);
        tmpD_F = data.response.per_false10(tmpX_F);
        tmpD_C = data.response.per_false10(tmpX_C);
% *** 08/23/2016 HS *** ***************************************************
% *** 02/03/2016 HS ***
        % plot
...
% *** 08/23/2016 HS *** ***************************************************
% plot dots for Hits/Miss/FA/CR.
        plot(hAX(2), tmpX_H, tmpD_H, 'bo', 'MarkerFaceColor','b'   );
        plot(hAX(2), tmpX_M, tmpD_M, 'bo', 'MarkerFaceColor','none');
        plot(hAX(2), tmpX_F, tmpD_F, 'ro', 'MarkerFaceColor','r'   );
        plot(hAX(2), tmpX_C, tmpD_C, 'ro', 'MarkerFaceColor','none');
% *** 08/23/2016 HS *** ***************************************************
        hold(hAX(2), 'off');
        % set color
        set(hLineH, 'Color', 'b');
...
%         set(hLineD, 'LineStyle', ':');
        ...
        if data.response.p_targ_after_cr > 0 || data.response.p_ntarg_after_fa > 0
            ...
            set(hLineD2, 'Color', 'k');
            % set line style
            set(hLineF2, 'LineStyle', ':');
            set(hLineD2, 'LineStyle', ':');
            hold(hAX(1), 'off');
            hold(hAX(2), 'off');
        end
...
% *** 05/20/2016 HS *** ***************************************************
% Moved to "turn off grating after response.target_time".
% % *** 03/14/2016 HS *** ***************************************************
%     data.response.aborted(end+1) = 0; % vis stim copleted: not aborted.
% % *** 03/14/2016 HS *** ***************************************************
% *** 05/20/2016 HS *** ***************************************************
...
% *** 08/23/2016 HS *** ***************************************************
% Add trial number to plot in performance fig.
            data.response.n_hits_trlN(end+1) = length(data.response.n_overall);
% *** 08/23/2016 HS *** ***************************************************
% *** 07/23/2016 HS *** ***************************************************
% moved back here from after response.target_time,
% so that data.response.n_overall and data.response.aborted get updated
% around the same time to avoid the error of finding the last hit trial in
% the end of psychsr_dscrim_HS.m script.
            data.response.aborted(end+1) = 0; % vis stim copleted: not aborted.
% *** 07/23/2016 HS *** ***************************************************
...
% *** 08/23/2016 HS *** ***************************************************
% Add trial number to plot in performance fig.
            data.response.n_false_trlN(end+1) = length(data.response.n_overall);
% *** 08/23/2016 HS *** ***************************************************
% *** 07/23/2016 HS *** ***************************************************
% moved back here from after response.target_time,
% so that data.response.n_overall and data.response.aborted get updated
...
% *** 05/20/2016 HS *******************************************************
% replace this with Gerald's response.p_ntarg_after_fa
% 
%     % *** 12/03/2015 HS ***
...
%     loop.hide_stim = 1;   
% end
if psychsr_timed_event(loop,3,data.stimuli.on_time(loop.stim))...
        && ~(data.response.nt_on_timeout == 1 && data.stimuli.orientation(loop.stim) ~= data.response.t_ori);
    loop.hide_stim = 1;
% % *** 03/29/2016 HS *** ***************************************************
%     if data.response.p_sound_help > rand % play click sound for help
%         fprintf('%s BEEP\n',datestr(loop.prev_flip_time/86400,'MM:SS'));
% %         psychsr_sound(6);
%         psychsr_sound(2);
%         data.response.helped(end+1) = 1;
%     else
%         data.response.helped(end+1) = 0;
%     end
% % *** 03/29/2016 HS *** ***************************************************

% *** 07/23/2016 HS *** ***************************************************
% Moved to 4 positions of beginning of ITI when data.response.n_overall
% gets updated, so that data.response.aborted also gets updated around
% the same time, to avoid the error of finding the last hit trial in
% the end of psychsr_dscrim_HS.m script.
% % *** 05/20/2016 HS *** ***************************************************
% % moved here from ITI onset.
%     data.response.aborted(end+1) = 0; % vis stim copleted: not aborted.
% % *** 05/20/2016 HS *** ***************************************************
% *** 07/23/2016 HS *** ***************************************************
...
% *** 03/22/2016 HS *** ***************************************************
% *** 05/20/2016 HS *** ***************************************************
if psychsr_timed_event(loop,3,data.stimuli.on_time(loop.stim)+0.1)...
        && ~(data.response.nt_on_timeout == 1 && data.stimuli.orientation(loop.stim) ~= data.response.t_ori);
    if data.response.p_sound_help > rand % play click sound for help
        fprintf('%s BEEP\n',datestr(loop.prev_flip_time/86400,'MM:SS'));
        psychsr_sound(2);
        data.response.helped(end+1)      = 1;
        data.response.helped_time(end+1) = loop.prev_flip_time;
    else
        data.response.helped(end+1)      = 0;
        data.response.helped_time(end+1) = loop.prev_flip_time;
    end
...
% *** 07/23/2016 HS *** ***************************************************
                                % play short noise
                                if data.response.abortTrlStimLick
                                    psychsr_sound(12);
                                end
% *** 07/23/2016 HS *** ***************************************************
                            end
...
%                     fprintf(' GRACE\n')
                    fprintf(' DURING STIM\n')
                    if data.response.abortTrlStimLick
                        fprintf('TURNED OFF STIM\n')
                        loop.hide_stim = 1; % hide stim
                        % record stim time.
                        data.presentation.stim_times(loop.stim) = loop.prev_flip_time;
% *** 03/22/2016 HS *** ***************************************************
                        % play noise
%                         psychsr_sound(1); % *** 05/20/2016 HS
                        psychsr_sound(12);  % Changed to short noise 05/20/2016 HS
% *** 03/22/2016 HS *** ***************************************************
% *** 03/14/2016 HS *** ***************************************************
                        data.response.aborted(end+1) = 1; % aborted trial.
% *** 03/14/2016 HS *** ***************************************************
                        loop.stim = loop.stim+1; % increment to ITI.
% *** 05/20/2016 HS *** ***************************************************
                        if data.stimuli.orientation(loop.stim-1) == data.response.t_ori
% Repeat same stim if data.response.p_targ_after_miss
                            if rand < data.response.p_targ_after_miss
                                data.stimuli.contrast   (loop.stim+3-mod(loop.stim,3)) = data.stimuli.contrast   (loop.stim-mod(loop.stim,3));
                                data.stimuli.orientation(loop.stim+3-mod(loop.stim,3)) = data.stimuli.orientation(loop.stim-mod(loop.stim,3));
                                data.stimuli.temp_freq  (loop.stim+3-mod(loop.stim,3)) = data.stimuli.temp_freq  (loop.stim-mod(loop.stim,3));
                                tmpIdx = find(fliplr(diff(data.response.n_hits)));
                                if isempty(tmpIdx); tmpIdx = length(data.response.n_hits); end
                                disp(['NEXT STIM: TARGET ( *** REPEAT ',num2str(tmpIdx(1)), ')']);
                            end
% *** 05/26/2016 HS *** ***************************************************
% Repeat same stim if data.response.block_Targets
                            if rand < data.response.block_Targets
                                data.stimuli.contrast   (loop.stim+3-mod(loop.stim,3)) = data.stimuli.contrast   (loop.stim-mod(loop.stim,3));
                                data.stimuli.orientation(loop.stim+3-mod(loop.stim,3)) = data.stimuli.orientation(loop.stim-mod(loop.stim,3));
                                data.stimuli.temp_freq  (loop.stim+3-mod(loop.stim,3)) = data.stimuli.temp_freq  (loop.stim-mod(loop.stim,3));
                                disp('NEXT STIM: NONTARGET');
...
% Repeat same stim if data.response.p_ntarg_after_fa
                            if rand < data.response.p_ntarg_after_fa
                                data.stimuli.contrast   (loop.stim+3-mod(loop.stim,3)) = data.stimuli.contrast   (loop.stim-mod(loop.stim,3));
                                data.stimuli.orientation(loop.stim+3-mod(loop.stim,3)) = data.stimuli.orientation(loop.stim-mod(loop.stim,3));
                                data.stimuli.temp_freq  (loop.stim+3-mod(loop.stim,3)) = data.stimuli.temp_freq  (loop.stim-mod(loop.stim,3));
...
% *** 05/26/2016 HS *** ***************************************************
% Repeat same stim if data.response.block_NonTargets
                            if rand < data.response.block_NonTargets
                                data.stimuli.contrast   (loop.stim+3-mod(loop.stim,3)) = data.stimuli.contrast   (loop.stim-mod(loop.stim,3));
                                data.stimuli.orientation(loop.stim+3-mod(loop.stim,3)) = data.stimuli.orientation(loop.stim-mod(loop.stim,3));
                                data.stimuli.temp_freq  (loop.stim+3-mod(loop.stim,3)) = data.stimuli.temp_freq  (loop.stim-mod(loop.stim,3));
                                disp('NEXT STIM: NONTARGET');
...
% *** 07/23/2016 HS *** ***************************************************
                ...
                if data.response.abortTrlStimLick
                    ...
                    psychsr_sound(12);  % Changed to short noise 05/20/2016 HS
                    data.response.aborted(end+1) = 1; % aborted trial.
                    loop.stim = loop.stim+2; % increment to ITI.
                ...
                end % if data.response.abortTrlStimLick
% *** 07/23/2016 HS *** ***************************************************
