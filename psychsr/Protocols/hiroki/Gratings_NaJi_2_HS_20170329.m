function Gratings_NaJi_2_HS_20170329(type)

% *** 03/29/2017 HS ***
% Modified from Gratings_mouse_SMS_HS_20170322.m and
%               Gratings_3_HS_20170329.m
% Added randomized option.
% *** 03/29/2017 HS ***
...
% Grating parameters.
Params.Nrepeats = 10;
Params.Nori = 8; % 45 degree apart
Params.contrast        = 1;
Params.rand_FLG = 1; % 0: clockwise, 1: randomized
...
% Params.spatialFrequency_cpd = 4; % cycles per degree
Params.spatialFrequency_cpd = 0.07; % cycles per degree

% % flashing
% Params.cyclespersecond = 5; % temporal frequency
% Params.spatialFrequency_cpd = 0.0001;%4; % cycles per degree

% % SCREEN
% Params.screen_distance_cm = 1;%10;
Params.screen_distance_cm = 7.5;
Params.screen_width_cm = 15.4;
Params.screen_width_pixels = 1024;
Params.pixels_per_degree = psychsr_calibrate_screen_HS(Params.screen_width_cm, Params.screen_distance_cm, Params.screen_width_pixels);

Params.sf_cyclesperpixel = Params.spatialFrequency_cpd / Params.pixels_per_degree; % spatial frequency in cycles per pixel
...
if strcmp(name,'ACHR2')
    ...
    savePath = 'C:\Users\Hiroki\Dropbox (MIT)\3P1\VisStimParams\';
else
    mypath='C:\Dropbox\MouseAttention\Matlab\psychsr\Protocols\RG_2p4Scripts';
    savePath = 'C:\Dropbox\3P1\VisStimParams\';
end
cd (mypath)
imageFlag = 0;
...
Params.NN  = GenerateCombinations_for_dendImaging_HS(Params.Nori, Params.sf_cyclesperpixel, Params.Nrepeats, Params.rand_FLG);
V = {'G'};
DisplaySequence = repmat(V,1,Params.Nrepeats);
% *** 09/19/2016 HS *** ***************************************************
...
% *** 03/29/2017 HS *** ***************************************************
% SAVE Parameters
saveFileName = [savePath, 'VisStimParams', ...
    datestr(now, '_yyyymmdd_HHMMSS'), '.mat'];
...
if keyIsDown == 1 && keyCode(esc) == 1
    ...
    for v = 1:length(DisplaySequence)
        ...
        elseif strcmpi( currentPosition, 'G') == 1
            ...
            vbl = Screen('Flip',win); % starting time point. All time points refer back to this.
            vbl0 = vbl;
            Nnew = squeeze( Params.NN(:,ctg,:) );
            displayGratingSequence_HS(rotateMode,Nnew,texgray, Params, waitframes, ifi, vbl,vbl0, win, Params.Nori)
            if imageFlag
...
function NN = GenerateCombinations_for_dendImaging_HS(NumOrient, sf_cyclesperpixel, Nrepeats, rand_FLG)
