function data = gpi_size_randori()
...
% end
% 2P3: 0.1 ~= 7 deg
centerpos = [0.5 0.5]; % x y from top left
...
% order = repmat([1:length(rad)],1,repeats);
order = repmat([1:length(rad) length(rad):-1:1],1,repeats/2);
% order = randomizeStims(mfilename('fullpath'),length(rad),repeats);
...
for i = 1:repeats*length(rad)	
	...
	radius(k+1:k+noris) = repmat(rad(order(i)),1,noris);
end
