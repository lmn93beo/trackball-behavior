...
order = randomizeStims(mfilename('fullpath'),length(contrasts)*length(orients),repeats);

total_duration = repeats*length(orients)*length(contrasts)*(offTime+onTime);
disp(total_duration)
%% constant parameters
screen.keep = 1;
...
%     for j = 1:length(orients)
        ...
        spat_freq(k) = sfs;
        temp_freq(k) = tfs;
        contrast(k) = contrasts(ceil(order(i)/length(orients)));
		pos{k} = centerpos;
		radius(k) = rad;
        k = k+1;
%     end
