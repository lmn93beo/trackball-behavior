function new_loop = psychsr_discrim_feedback_RR(loop)
global data;
...
if loop.frame - loop.new_stim == 1 && mod(loop.stim,3) == 0
    ...
    if data.stimuli.target(loop.stim) == 1, fprintf(' T');
    else fprintf(' NT'); end;
...
if loop.frame - loop.new_stim == 1 && mod(loop.stim,3) == 1
    ...
    if data.stimuli.target(loop.stim-1) == 1
...
%                 end
%                 data.stimuli.target(loop.stim+3-mod(loop.stim,3))=data.response.t_ori;
...
if psychsr_timed_event(loop,3,data.response.target_time)...
        && ~(data.response.nt_on_timeout == 1 && data.stimuli.target(loop.stim) ~= 1);
    loop.hide_stim = 1;   
end
...
if data.stimuli.target(loop.stim)==1
    if data.response.auto_reward ~= 0 && psychsr_timed_event(loop,3,data.response.auto_reward_time)        
        if data.response.auto_reward == -2
            fprintf('%s BEEP\n',datestr(loop.prev_flip_time/86400,'MM:SS'));
            psychsr_sound(6);            
        elseif isempty(data.response.rewards) || max(data.response.licks)-max(data.response.rewards) > 0 && ...
