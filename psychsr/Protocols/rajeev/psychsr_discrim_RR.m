function data = psychsr_discrim_RR()
%% TRIAL STRUCTURE
% delay --> response --> iti
...
tmovies = dir([mfolder '\Target\*.mat']); tmovies = {tmovies(:).name};
tmovies = cellfun(@(x) {[mfolder '\Target\' x]},tmovies);
ntmovies = dir([mfolder '\NonTarget\*.mat']); ntmovies = {ntmovies(:).name};
ntmovies = cellfun(@(x) {[mfolder '\NonTarget\' x]},ntmovies);
...
response.feedback_fn = @psychsr_discrim_feedback_RR;
...
response.auto_reward = 5000;       % # of free rewards if no lick
response.auto_reward_time = 3;% time after grating onset for auto reward
...
switch mouse              
    case 108
        response.mode = 1; % quinine
        psychsr_discrim_stages_RR(30);
        response.target_time = 4; % stimulus on time
        resp_delays = response.target_time; % stimulus-response delay
...
for i = 1:num_loops
    ...
    target(k) = targets(i);
    if targets(i) == 1 % target
        movie_file{k} = tmovies{1};
    else
        movie_file{k} = ntmovies{1};
    end
    contrast(k) = 1;
    laser_on(k) = lasers(i);
    k = k+1;
...
if reload == 1
    ...
    stimuli = psychsr_zip(num_stimuli,total_duration,stim_type,duration,movie_file,target,contrast,cue_tone,...
...
targ = data.stimuli.target(3:3:3*length(data.response.n_overall))==1;
...
%% save
date = clock;
folder = sprintf('../behaviorData/Rajeev/mouse %04d',mouse);
