function data = psychsr_discrim_GS()
%% TRIAL STRUCTURE
% delay --> response --> iti
...
response.feedback_fn = @psychsr_discrim_feedback_GS;
...
response.p_ntarg_after_fa = 0;  % prob force next stim to be NTarg after FA
...
%         response.p_targ_after_cr = 0.8;   % prob force next stim to be Targ after CR
%         response.p_ntarg_after_fa = 0.8;  % prob force next stim to be Targ after FA
        
    case 125
        response.mode = 6; % quinine
        psychsr_discrim_stages_GS(10);
        [amt time b] = psychsr_set_reward(6);
