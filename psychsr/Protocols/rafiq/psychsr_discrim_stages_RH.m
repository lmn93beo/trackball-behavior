function psychsr_discrim_stages_RH(stage)
...
response.iri = 0.5;
response.response_time = 3;   % response window
response.target_time = 3;     % time grating is displayed        
...
response.stagenum = 4;
ntarget = 15;                    % # of targets at beginning of session
...
response.stagenum = 7;
% if response.mode < 5 %takes away spout retraction
%     response.mode = 5;              % normal = 1; spout retract = 5
% end;
nt_speed = 2 %used to be 0; change back for other mice
        %}
    case {8, 'Engage: NT=1'}
        str = verbatim;
        %{
...
nt_speed = 2;                   % nontarget temporal frequency
response.target_time = 1.5;       % length of time grating is actually on
response.extend_onset = 0;      % time after grating onset
response.retract_onset = 1.5;   % time after grating onset (Inf = do not retract every trial)
stmdelay = [2];             % stimulus offset-response delay
resp_delays = stmdelay + response.target_time;    % stimulus-response delay
response.nt_on_timeout = 0;
blockdels = 1;
iti = 3;
...
response.extend_onset = 0;      % time after grating onset
response.retract_onset = 1.5;   % time after grating onset (Inf = do not retract every trial)
stmdelay = [0 3 6];             % stimulus offset-response delay
resp_delays = stmdelay + response.target_time;    % stimulus-response delay
response.nt_on_timeout = 0;
blockdels = 1;
iti = 3;
