...
%     idx = mod(i-1,length(order))+1;
	...
	duration(i*2-1) = onTime;
	stim_type{i*2-1} = 'fillrect';
	
	duration(i*2) = offTime;
    stim_type{i*2} = 'blank';  
    
    contrast(i*2-1) = contrasts;	
	% [xmin ymin ymin ymax] normalized to 1	
    rect{i*2-1} = rects(order(i),:);
	color(i*2-1) = colors(order(i));
