function new_loop = psychsr_discrim_feedback_MH(loop)
global data;
...
%% beginning of trial
 if loop.frame - loop.new_stim == 1 && mod(loop.stim,3) == 2
    ...
    if data.stimuli.cue_tone(loop.stim) ~= 0
        ...
        fprintf('%s CUE %d\n',datestr(loop.prev_flip_time/86400,'MM:SS'),data.stimuli.cue_tone(loop.stim)-13)
    end
end
...
if psychsr_timed_event(loop,3,data.response.target_time)...
    ...
    loop.hide_stim = 1; %0 grating during delay
    data.stimuli.temp_freq(loop.stim)=0; 
end
...
if data.response.auto_reward ~= 0 && psychsr_timed_event(loop,3,data.stimuli.response_delay(loop.stim)) && data.stimuli.orientation(loop.stim)==data.response.t_ori
...
%      fprintf('%s %4d',datestr(loop.prev_flip_time/86400,'MM:SS'),length(data.response.licks));
     fprintf('%4d',length(data.response.licks));
...
%                 end
    elseif data.response.auto_reward == 0 
        ...
        switch mod(loop.stim,3)
            case 0
                ...
                if (loop.prev_flip_time > data.presentation.stim_times(end)+data.response.grace_period+data.stimuli.response_delay(loop.stim))
                    % REWARD
                    if data.stimuli.orientation(loop.stim) == data.response.t_ori || data.stimuli.orientation(loop.stim) == data.response.nt_ori
