function data = psychsr_discrim_MH()
%% TRIAL STRUCTURE
% delay --> response --> iti
...
response.feedback_fn = @psychsr_discrim_feedback_MH;
presentation.frame_rate = 30;
presentation.lag = 0;
blackblank = 0;                 % blank screen is black or gray?

spat_freq = 0.04;               % spatial frequency in cpd
...
delay = 1;                      % soud cue delay duration
response.response_time = 2.5;   % was response window
response.target_time = 1.5;     % time grating is displayed
response.grace_period = 0.2;    % time during grating that licks don't matter
response.lick_threshold = 1;    % number of licks required to trigger "response"

nt_ori = 0.01;                  % nontarget orientation(s)
response.nt_ori = nt_ori;
...
response.nt_on_timeout = 0;     % was 1, leave nontarget on during timeout?
...
blockdels = 1;                  % vary delays in blocks of this size
firstbdel = 1;                  % which delay(s) to use in first block? (use integer)
...
% reward block control (RH)
per_high = 0.5;
cuesounds = [13 14];            % sound to play for low/high
rewardamounts = [3 12];         % amount to give for low/high
...
%         case 1003
%         %         psychsr_discrim_stages_RH(5);
%         %         [amt time b] = psychsr_set_reward(8);
%         %         response.response_time = 4.5;   % was response window
%         %         response.target_time = 1.5;     % time grating is displayed
%         %         response.grace_period = 3.5;   % was time during grating that licks don't matter
%         %         iti = 3.5; %3.5
%         %         ntarget = 3; %250
...
%         %         response.auto_reward_time = 3.5;
%         %         cue_sound = 0;
%         %         response.lick_threshold = 1;
%         
%         psychsr_discrim_stages_RH(5);
%         rewardamounts = [-12 8]; %[nontarg amout, targ amount];
%         cuesounds = [13 15]; %sound for rewad/no reward
%         [amt time b] = psychsr_set_reward(rewardamounts);
%         response.response_time = 2.5;   % was response window
%         response.target_time = 1.5;     % time grating is displayed
%         response.grace_period = 0;   % was time during grating that licks don't matter
%         iti = 10; %3.5
%         ntarget = 300; %250
%         response.extend_iti = 0;
%         extend_iti = 0;
%         per_targ = 0.7;                 % percent of stimuli that are targets
%         nt_speed = 2;
%         response.punish_timeout = 0;
%         contrasts = 1;
%         response.auto_reward = 200;
%         response.auto_reward_time = 0; %from beginning of delay
%         blockmins = 10;
%         per_high = 0.5;   
%         data.params.perNoReward = 0;
 ...
 case 1032 % pavlovian
        ...
        psychsr_discrim_stages_RH(5);
        rewardamounts = [-12 10]; %[nontarg amout, targ amount];
        tone_time = 3;
        cuesounds = [13 15]; %[13 15]; %sound for rewad/no reward
        delay = 1; %delay between tone and visual stimulus
        [amt time b] = psychsr_set_reward(rewardamounts);
        response.response_time = 0;   % was response window
        response.target_time = 2;     % time grating is displayed
        response.grace_period = 0;   % was time during grating that licks don't matter
        resp_delays = [3]; %time from grating onse
        iti = 7; %3.5
        ntarget = 10; %250
        response.extend_iti = 0;
        extend_iti = 0;
        ...
        response.punish_timeout = 0;
        contrasts = 1;
        response.auto_reward = 1000;
        response.auto_reward_time = 4; %from beginning of delay
        blockmins = 20;
        per_high = 1;   
        data.params.perNoReward = 0;
    ...
    case 1030 % post pavlovian

        
        psychsr_discrim_stages_RH(5);
        rewardamounts = [-12 8]; %[nontarg amout, targ amount];
        tone_time = 3;
        cuesounds = [15 13]; %[13 15]; %sound for rewad/no reward
        delay = 1; %delay between tone and visual stimulus
        [amt time b] = psychsr_set_reward(rewardamounts);
        response.response_time = 0;   % was response window
        response.target_time = 2;     % time grating is displayed
    ...
    case 1035 % pavlovian
        ...
        contrasts = 1;
        response.auto_reward = 1000;
        response.auto_reward_time = 4; %from beginning of tone
        blockmins = 30;
        per_high = 1;   
        data.params.perNoReward = 0;
    ...
    case 1004
        ...
        psychsr_discrim_stages_RH(5);
        rewardamounts = [-12 5]; %[nontarg amout, targ amount];
        cuesounds = [13 15]; %sound for rewad/no reward
        delay = 1;
        [amt time b] = psychsr_set_reward(rewardamounts);
        response.response_time = 2.5;   % was response window
        response.target_time = 2.5;     % time grating is displayed
        response.grace_period = 0;   % was time during grating that licks don't matter
        resp_delays = [0.5 0.5];
        iti = 6; %3.5
        ntarget = 5; %250
    ...
    case 1008
        ...
        psychsr_discrim_stages_RH(5);
        rewardamounts = [-12 12]; %[nontarg amout, targ amount];
        cuesounds = [15 13]; %was [13 15], reversal started 6.23.2016 %sound for rewad/no reward
        delay = 1;
        [amt time b] = psychsr_set_reward(rewardamounts);
        response.response_time = 2.5;   % was response window
        response.target_time = 2.5;     % time grating is displayed
    ...
    case 1013
        ...
        psychsr_discrim_stages_RH(5);
        rewardamounts = [-12 6]; %[nontarg amout, targ amount];
        cuesounds = [15 13]; %was [13 15], reversal started 9.15.2016 %sound for rewad/no reward
        delay = 1;
        [amt time b] = psychsr_set_reward(rewardamounts);
        response.response_time = 2.5;   % was response window
        response.target_time = 2.5;     % time grating is displayed
...
%         psychsr_discrim_stages_RH(5);
%         rewardamounts = [-12 6]; %[nontarg amout, targ amount];
%         cuesounds = [13 15]; %was [13 15], reversal started 9.15.2016 %sound for rewad/no reward
%         delay = 3;
%         [amt time b] = psychsr_set_reward(rewardamounts);
%         response.response_time = 2.5;   % was response window
%         response.target_time = 2.5;     % time grating is displayed
...
%         
    ...
    case 0021
       ...
       psychsr_discrim_stages_RH(5);
        ...
        response.grace_period = 0;   % was time during grating that licks don't matter
        resp_delays = [2.5 2.5]; % delay period;
        response.lick_threshold = 1;
    ...
    case 0018
        ...
        sound.tone_amp = 0.5;
        [amt time b] = psychsr_set_reward(rewardamounts);
        response.response_time = 0.5;   % was response window
        response.target_time = 1.5;     % time grating is displayed
        
        response.grace_period = 0;   % was time during grating that licks don't matter
        resp_delays = [2.0 2.0]; % delay period;
        response.lick_threshold = 1;
    ...
    case 1
        psychsr_discrim_stages_RH(7);
        [amt time b] = psychsr_set_reward(8.4);
        response.response_time = 2;   % was response window
        response.target_time = 1.5;     % time grating is displayed
        response.grace_period = 1;   % was time during grating that licks don't matter
        response.mode = 6;
    ...
    case 2
        ...
        iti = 2.5;
        per_targ = 0.2;
        %     case 2 %for random reward delay
        %         psychsr_discrim_stages_RH(7);
        %         [amt time b] = psychsr_set_reward(3);
        %         response.grace_period = 0; % starts after resp_delay
        %         response.response_time = 1;   % was response window
        % %         response.retract_onset = response.response_time;
        %         response.target_time = 1.5;     % time grating is displayed
        %
        % %         stmdelay = [0 1 2];             % stimulus offset-response delay
        % %         resp_delays = stmdelay + response.target_time;    % stimulus-response delay
        % %         response.spout_time = 0; % make spout
        % %         firstbdnum = 5;                 % number in first block
        %
    case 0
        psychsr_discrim_stages_RH(2);
        [amt time b] = psychsr_set_reward(4);
        response.iri = 1;
        cue_sound = 0;
    ...
    case 7
        ...
        %         response.response_time = 2.5;   % was response window
        %         response.target_time = 1.5;     % time grating is displayed
        %         response.grace_period = 1.5;   % was time during grating that licks don't matter
        %         response.auto_reward = 0;
        %         response.auto_reward_time = 1.5;
        %         response.extend.iti = 1;
        %         nt_speed = 2;
        %
        
        psychsr_discrim_stages_RH(5);
        rewardamounts = [3 12];
        cuesounds = [15 13];            % sound to play for low/high
        [amt time b] = psychsr_set_reward(rewardamounts);
        response.response_time = 4.5;   % was response window
        response.target_time = 1.5;     % time grating is displayed
        response.grace_period = 3.5;   % was time during grating that licks don't matter
    ...
    case 9
        ...
        %
    case 8
        %For non-behavior imaging
        psychsr_discrim_stages_RH(5);
        [amt time b] = psychsr_set_reward(4);
        response.iri = 1;
        response.extend_iti = 0;
        response.target_time = 4;
        iti = 3;
        response.iti = 4;
        ntarget = 0;
        cue_sound = 0;
    ...
    case 10
        ...
        cuesounds = [13 15]; %sound for rewad/no reward
        [amt time b] = psychsr_set_reward(rewardamounts);
        response.response_time = 3;   % was response window
        response.target_time = 1.5;     % time grating is displayed
        response.grace_period = 0;   % was time during grating that licks don't matter
        iti = 3.5; %3.5
        ntarget = 300; %250
    ...
    case 112
        psychsr_discrim_stages_RH(5);
        rewardamounts = [3 12];
        cuesounds = [13 15];            % sound to play for low/high
        [amt time b] = psychsr_set_reward(rewardamounts);
        response.response_time = 4.5;   % was response window
        response.target_time = 1.5;     % time grating is displayed
        response.grace_period = 3.5;   % was time during grating that licks don't matter
...
reward_amt = zeros(1,num_stimuli);
...
%% randomize reward amounts
%rewards = (rand(1,num_loops)<per_high)+1; % 1 means low reward, 2 means high

% high reward for non-target
rewards = ones(1,length(targets))+1;
rewards(targets == 2) = 1;

if data.params.perNoReward 
    noReward = rand(1,length(targets)) < data.params.perNoReward;
    noReward(1:10) = 0; 
    rewards(noReward(1:length(targets)) == 1) = 3;
end
...
for i = 1:num_loops
    ...
    laser_on(k) = lasers(i);
    if rewards(i) > 0
        cue_tone(k) = cuesounds(rewards(i));
    end
    k = k+1;
    
    duration(k) = response.response_time+max(resp_delays)+1.5;
    stim_type{k} = 'grating';
    spat_freq(k) = sfs;
    rand_phase(k) = 1;
...
if reload == 1
    ...
    stimuli = psychsr_zip(num_stimuli,total_duration,stim_type,duration,contrast,...
        orientation,spat_freq,temp_freq,cue_tone,reward_amt,...
...
%% save
date = clock;
folder = sprintf('C:\Users\Surlab\Dropbox\Rafiq\behaviorData\ mouse %04d',mouse);
