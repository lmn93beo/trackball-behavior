...
folder = [cd ,'\psychsr\Helpers\'];
if isinf(radius)
    % Load standard grating shader:
    gratingShader = LoadGLSLProgramFromFiles([folder 'BasicSquareGratingShader'], 1);
else
    % Load grating shader with circular aperture support:
    gratingShader = LoadGLSLProgramFromFiles({[folder 'BasicSquareGratingShader.vert.txt'], [folder 'ApertureSquareGratingShader.frag.txt']}, 1);
end
