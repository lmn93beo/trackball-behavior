...
switch data.mouse
    ...
    case 003
        ...
        data.params.threshold = [10 10]; % in degrees, can have diff thresholds for left/right
        data.params.responseTime = 3;   % maximum reaction time
...
%         data.params.contrast = 1;%[0 ones(1,9)]; % 10% of trials are blank
%         data.params.contrast = [0 16 16 32 32 64 64 64]/100; % easy variable contrast
%         data.params.contrast = [8 16 32 64]/100;
%        data.params.contrast = [0 4 4 8 8 16 16 32 32 64]/100; % hard variable contrast
%         data.params.contrast = [0 0 64 64 64 64 64 64 64]/100; % hard variable contrast
...
%         data.params.antibiasSwitch = 1; % probability that antibias will switch after a correct trial
%         data.params.antibiasNumCorrect = [1]; % number of correct responses before forced switch
%         data.params.antibiasConsecutive = 3; % must be consecutive
%         
%         data.params.responseTime = 3;   % maximum reaction time
%         data.params.punishDelay = 3;
%                 
%         data.params.threshold = [10 7]; % in degrees, can have diff thresholds for left/right
%         data.params.reversalFreq = 0;
%         
%         data.params.flashStim = [0.2];   % flash stimulus for this long (if 0, do not flash)
%         data.params.goDelay = [0]; %[0.5 2]; 
...
%         % laser parameters
%         data.params.laser = 0;          % laser on
%         data.params.laser_time = 3;     % duration in s
%         data.params.laser_start = -0.3;    % time after trial start
%         data.params.perLaser = 0.2;    % percent laser trials
%         data.params.noLaser = 10;        % number of no-laser trials at beginning
        
    case 008
        data.params.numTrials = 200;
        ...
        data.params.antibiasNew = 0;    % overrides other antibias algorithm. Tries to compensate for both raw bias and stay/switch biases
        data.params.contrast = 0.3; %[0 4 4 8 8 16 16 32 32 64]/100; 
        
        data.params.flashStim = [0.2];   % flash stimulus for this long (if 0, do not flash)
        data.params.goDelay = [0.3]; %[0.5 2]; 
        data.params.early_cue = 1; % cue --> delay --> stim (otherwise delay-->cue-->stim)        
        
        % laser parameters
        data.params.laser = 1;          % laser on
        data.params.laser_time = 3.3;     % duration in s
        data.params.laser_start = -0.3;    % time after trial start
        data.params.perLaser = 0.30;    % percent laser trials
        data.params.noLaser = 0;        % number of no-laser trials at beginning
...
%         data.params.reward = [0 4];
%         data.params.rewardProb = [0 1];
%         data.params.blockSize = [5 7];%[10 20]; % min/max block size
%         if rand > 0.5
%             data.params.blockSeq = [3 1 3 2]; % sequence of blocks  ;[3 1 3 2] 
%         else
%             data.params.blockSeq = [3 2 3 1]; % sequence of blocks  [3 2 1 2 3 1 2 1]
...
%         data.params.timeout = 7;
    ...
    case 009
        ...
        data.params.responseTime = 1;   % maximum reaction time
        data.params.punishDelay = 0;
        
        data.params.freeBlank = 0;
        data.params.threshold = [20 20]; % in degrees, can have diff thresholds for left/right
        data.params.reversalFreq = 0;
        
        data.params.flashStim = [0.04];   % flash stimulus for this long (if 0, do not flash)
        data.params.goDelay = [1.8 4]; %[0.5 2]; 
        data.params.early_cue = 1; % cue --> delay --> stim (otherwise delay-->cue-->stim)
        
        data.params.contrast = [16 16 32 32 64 64 100 100]/100; % hard variable contrast
        %data.params.contrast = 0.3;
        
         % action value parameters        
%         data.params.reward = [0 4];
%         data.params.rewardProb = [0 0.9];
%         data.params.blockSize = [10 12];%[10 20]; % min/max block size
%         if rand > 0.5
%             data.params.blockSeq = [3 1 3 2]; % sequence of blocks  ;[3 1 3 2] 
%         else
%             data.params.blockSeq = [3 2 3 1]; % sequence of blocks  [3 2 1 2 3 1 2 1]
...
%         data.params.timeout = 7;
        ...
        data.params.antibiasNew = 0;    % overrides other antibias algorithm. Tries to compensate for both raw bias and stay/switch biases
        data.params.laser = 1;          % laser on
        data.params.laser_time = 0.1;     % duration in s
        data.params.laser_start = 0; % time after trial start
        data.params.perLaser = 0.30;    % percent laser trials
        data.params.noLaser = 0;        % number of no-laser trials at beginning
    ...
    case 010
        ...
        data.params.goDelay = [1.8 4]; %[0.5 2]; 
        data.params.early_cue = 1; % cue --> delay --> stim (otherwise delay-->cue-->stim)
        data.params.contrast = 0.3; %[4 4 8 8 16 16 32 32 64]/100;
        data.params.timeout = 7;
        data.params.preCueEarlyAbort = 0;
    ...
    case 011
        ...
        data.params.responseTime = 1;   %maximum reaction time
        data.params.punishDelay = 0;
        data.params.itiDelay = 2;
        
        data.params.freeBlank = 0;
        data.params.threshold = [25 25]; % in degrees, can have diff thresholds for left/right
        data.params.reversalFreq = 0;
        
        data.params.flashStim = [0.10];   % flash stimulus for this long (if 0, do not flash)
        data.params.goDelay = [1.5 5]; %[0.5 2]; 
        data.params.minGoDelay = 0.3;
        data.params.early_cue = 1; % cue --> delay --> stim (otherwise delay-->cue-->stim)
        data.params.omitEarlyTone = 0;
        data.params.contrast = [4 4 8 8 16 16 32 32 64]/100; %0.3;
        data.params.timeout = 7;
        ...
        data.params.antibiasNew = 0;    % overrides other antibias algorithm. Tries to compensate for both raw bias and stay/switch biases
        data.params.laser = 0;          % laser on
        data.params.laser_time = 0;     % duration in s
        data.params.laser_start = -0.3; % time after trial start
    ...
    case 012
        ...
        data.params.responseTime = 1.5;   % maximum reaction time
        data.params.punishDelay = 2;
        ...
        data.params.flashStim = [0.04];   % flash stimulus for this long (if 0, do not flash)
        data.params.goDelay = [1 3]; %[1.5 3]; %[0.5 2]; 
        data.params.minGoDelay = 0.5;
        data.params.early_cue = 1; % cue --> delay --> stim (otherwise delay-->cue-->stim)
        data.params.contrast = 0.3;
        data.params.timeout = 7;
        ...
        data.params.antibiasNew = 0;    % overrides other antibias algorithm. Tries to compensate for both raw bias and stay/switch biases
        data.params.laser = 1;          % laser on
        data.params.laser_time = 0.9;     % duration in s
        data.params.laser_start = 0.05; % time after trial start
        data.params.perLaser = 0.3;    % percent laser trials
        data.params.noLaser = 10;        % number of no-laser trials at beginning
        
    case 013
        data.params.reward = [9];
        data.params.rewardProb = [1];
        data.params.freeChoice = 0;     % percentage of free choices
        data.params.perRight = 0.5;       % percentage of right trials (out of all non-freechoice)
        data.params.antibiasRepeat = 0; % probability that antibias will repeat after a wrong trial
        ...
        data.params.responseTime = 2.5;   % maximum reaction time
        data.params.punishDelay = 2;
        data.params.itiDelay = 2;
        
        data.params.goDelay = [1.5 3]; %[0.5 2]; 
        data.params.minGoDelay = 0.5;
        data.params.preCueEarlyAbort = 0;
        data.params.training = 0; % assymetric threshold
        ...
        data.params.contrast = 0.3;
        data.params.flashStim = 0.04;
        data.params.early_cue = 1;
        data.params.earlyCue_sound = 30;
    ...
    case 014
        ...
        data.params.antibiasSwitch = 0.5; % probability that antibias will switch after a correct trial
        ...
        data.params.training = 1; % assymetric threshold        
        data.params.freeBlank = 0;
     ...
     case 015
        ...
        data.params.responseTime = 1;   % maximum reaction time
        data.params.punishDelay = 3; %2
        data.params.itiDelay = 2;
        
        data.params.freeBlank = 1;  
        data.params.training = 0;
        data.params.threshold = [17 17]; % in degrees, can have diff thresholds for left/right
        data.params.reversalFreq = 0;
        data.params.earlyCue_sound = 30;
        data.params.goDelay = 0.5;
        data.params.early_cue = 1;
     ...
     case 016
        ...
        data.params.antibiasNumCorrect = [1 2]; % number of correct responses before forced switch
        ...
        data.params.antibiasNew = 0;    % overrides other antibias algorithm. Tries to compensate for both raw bias and stay/switch biases
        data.params.laser = 0;          % laser on
        data.params.laser_time = 0.4;     % duration in s
        data.params.laser_start = -0.5; % time after trial start
        data.params.perLaser = 0.30;    % percent laser trials
        data.params.noLaser = 0;        % number of no-laser trials at beginning
      ...
      case 017
        ...
        data.params.rwdDeliveryDelay = 0;
        data.params.itiBlack = 0;
     ...
     case 018
        ...
        data.params.antibiasNumCorrect = [3 3]; % number of correct responses before forced switch
        data.params.antibiasConsecutive =  1; % must be consecutive
        
        data.params.responseTime = 1;   % maximum reaction time
        data.params.punishDelay = 2;
        
        data.params.goDelay = [0.3];
        data.params.early_cue = 1;
        data.params.itiDelay = 1;
        
        data.params.freeBlank = 0;
        data.params.threshold = [6 6]; %in degrees, can have diff thresholds for left/right
        data.params.training = 0;
        data.params.reversalFreq = 2;
    ...
    case 019
     ...
     case 020
        ...
        data.params.freeBlank = 0;
        data.params.threshold = [12 15]; %in degrees, can have diff thresholds for left/right
        data.params.training = 0;
        data.params.reversalFreq = 0;
        data.params.earlyCue_sound = 30;
     ...
     case 021
        ...
        data.params.freeBlank = 1;
        data.params.threshold = [14 14]; %in degrees, can have diff thresholds for left/right
        data.params.training = 0;
        data.params.reversalFreq = 0;
        data.params.earlyCue_sound = 30;
        data.params.flashStim = 0.2;
        ...
        data.params.contrast = [0.64];
        data.params.opp_contrast = [0 0.08 0.32 0.48 0.56];
        ...
        data.params.perLaser = 0.15;    % percent laser trials
    ...
    case 022
        ...
        data.params.freeBlank = 1;
        data.params.threshold = [16 16]; %in degrees, can have diff thresholds for left/right
        data.params.training = 0; 
        data.params.trainingSide = [1 1]; % multiplier for contra mvmts, make [0 0] for assym on both sides
        data.params.reversalFreq = 0;
        data.params.earlyCue_sound = 30;
        
        data.params.flashStim = 0.1;
        data.params.proOrienting = 1;
     ...
     case 23
        ...
        data.params.proOrienting = 0;
        data.params.flashStim = 0.05;
        data.params.earlyCue_sound = 30;
        
        data.params.contrast = 1; %[8 8 10 10 12 12 16 16 32 32]/100;
        ...
        data.params.perLaser = 0.3;    % percent laser trials
        data.params.noLaser = 0;        % number of no-laser trials at beginning
         data.params.laser_mode = 'pulses';
        data.params.laser_freq = 20; % frequency
        data.params.laser_pw = 10; % pulse width
     ...
     case 24
        ...
        data.params.trainingSide = [1 1];
        data.params.reversalFreq = 0;
        data.params.contrast = [0.16 .32 .64];
    ...
    case 26
        ...
        data.params.proOrienting = 0;
        data.params.flashStim = 0.3;
        data.params.earlyCue_sound = 30;
        data.params.contrast = 1;
        data.params.itiDelay = 2;
        data.params.extenddelay = 0; 
        
        data.params.rwdDeliveryDelay = [0.4];
        
   case 10000 %for new animals
        ...
        data.params.freeBlank = 0;
        data.params.threshold = [3 3]; %in degrees, can have diff thresholds for left/right
   ...
   case 28 
        ...
        data.params.freeBlank = 0;
        data.params.threshold = [8 8]; %in degrees, can have diff thresholds for left/right
        data.params.training = 0;
        data.params.trainingSide = [1 1];
        data.params.reversalFreq = 0;
   ...
   case 29 
    ...
    case 30 %for new animals
        ...
        data.params.laser_start = -1; % time after trial start
        data.params.perLaser = 0.35;    % percent laser trials
        data.params.noLaser = 0;        % number of no-laser trials at beginning
    ...
    case 31 
        ...
        data.params.freeBlank = 0;
        data.params.threshold = [18 18]; %in degrees, can have diff thresholds for left/right
        data.params.training = 0;
        data.params.trainingSide = [1 1];
        data.params.reversalFreq = 0;
    ...
    case 36 %for new animals
      ...
      case 33 %for new animals
       ...
       % Laser
        data.params.antibiasNew = 0;    % overrides other antibias algorithm. Tries to compensate for both raw bias and stay/switch biases
        data.params.laser = 0;          % laser on
        data.params.laser_time = 1;     % duration in s
               
        data.params.laser_start = 0.01; % time after trial start
        data.params.perLaser = 0.20;    % percent laser trials
        data.params.noLaser = 10;        % number of no-laser trials at beginning
      ...
      case 34 
        ...
        data.params.freeBlank = 0;
        data.params.threshold = [11 11]; %in degrees, can have diff thresholds for left/right
        data.params.training = 0;
        data.params.trainingSide = [1 1];
        data.params.reversalFreq = 2;
        ...
        data.params.antibiasNew = 0;    % overrides other antibias algorithm. Tries to compensate for both raw bias and stay/switch biases
        data.params.laser = 1;          % laser on
        data.params.laser_time = 1.5;     % duration in s
    ...
    case 39 
        ...
        data.params.reversalFreq = 0;
        data.params.contrast = [1];
        data.params.opp_contrast = 0; %[0 0.16 0.32 0.64 0.64];
        data.params.simultaneous = 1;
    ...
    case 41 %for new animals
        
        data.params.quitAfterMiss = 1000;
...
%         data.params.minGoDelay = 1;
        ...
        data.params.proOrienting = 0;
        data.params.flashStim = 0.2;
        data.params.contrast = [0.16 0.32 0.64];
        data.params.opp_contrast = [0]; %[0 0.16 0.16 0.32 0.56 0.64];
        data.params.earlyCue_sound = 30;
   ...
   case 43 %for new animals
     ...
     case 44 %for new animals
       ...
       case 45 %for new animals
        ...
        data.params.simultaneous = 0;
        data.params.opp_contrast = [0 0.16 0.16 0.32 0.56 0.64];
        ...
        data.params.laser = 0;          % laser on
        data.params.laser_blank_only = 0;
        data.params.laser_time = 1.3;     % duration in s for SC 0.3
        data.params.laser_mode = 'pulses';
        data.params.laser_freq = 20; % frequency
        data.params.laser_pw = 20; % pulse width
               
        data.params.laser_start = -0.3; % time after trial start for SC 0.15
        data.params.perLaser = 0.20;    % percent laser trials
        data.params.noLaser = 10;        % number of no-laser trials at beginning
        
     case 48 %for new animals
        ...
        data.params.laser = 0;          % laser on
        data.params.laser_blank_only = 0;
        data.params.laser_time = 1.30;     % duration in s for SC 0.3
...
%         data.params.laser_pw = 20; % pulse width
     ...
     case 49 %for new animals
        ...
        data.params.perRight = 0.5;       % percentage of right trials (out of all non-freechoice)
        data.params.antibiasRepeat = 0; % probability that antibias will repeat after a wrong trial
        data.params.antibiasSwitch = 0; % probability that antibias will switch after a correct trial1
...
%         data.params.laser_pw = 20; % pulse width
     ...
     case 51 %for new animals
        ...
        data.params.freeBlank = 0;
        data.params.threshold = [15 15]; %13 in degrees, can have diff thresholds for left/right
        data.params.training = 0;
        data.params.trainingSide = [1 1];
        data.params.reversalFreq = 0;
    ...
    case 5003 %for new animals
        ...
        data.params.freeBlank = 0;
        data.params.threshold = [1500 1500]; %13 in degrees, can have diff thresholds for left/right
        data.params.training = 0;
        data.params.trainingSide = [1 1];
        data.params.reversalFreq = 2;
        data.params.quitAfterMiss = 100000;
        data.params.timeout = 1000000;
      ...
      case 52 %for new animals
        ...
        data.params.freeBlank = 0;
        data.params.threshold = [13 13]; %in degrees, can have diff thresholds for left/right
        data.params.training = 0;
        data.params.trainingSide = [1 1];
        data.params.reversalFreq = 0;
...
%         data.params.laser_pw = 20; % pulse width
   ...
   case 55 %for new animals
        ...
        data.params.antibiasNumCorrect = [2 2]; % number of correct responses before forced switch
        data.params.antibiasConsecutive =  1; % must be consecutive
        ...
        data.params.freeBlank = 0;
        data.params.threshold = [7 7]; %in degrees, can have diff thresholds for left/right
        data.params.training = 1;
        data.params.trainingSide = [0.7 0.7];
        data.params.reversalFreq = 2;
...
%         data.params.laser_pw = 20; % pulse width
   ...
   case 59 %for new animals
        ...
        data.params.antibiasNumCorrect = [0 0]; % number of correct responses before forced switch
        data.params.antibiasConsecutive =  0; % must be consecutive
      ...
      case 60 %for new animals
         
        data.params.reward = [8];
        data.params.rewardProb = [1];
        data.params.freeChoice = 0;     % percentage of free choices
        data.params.perRight = 0.5;       % percentage of right trials (out of all non-freechoice)
        data.params.antibiasRepeat = 0.9; % probability that antibias will repeat after a wrong trial
        data.params.antibiasSwitch = 0.9; % probability that antibias will switch after a correct trial
    ...
    case {78}
     ...
     case {79}
        ...
        data.params.go_cue = 1;         % play go sound? (delay --> sound --> mvmt)
        data.params.noMvmtTime = 1;     % time after stimulus onset where movement is not counted (Before go sound)
        data.params.earlyAbort = 0;
...
% 
%         data.params.go_cue = 1;         % play go sound? (delay --> sound --> mvmt)
%         data.params.noMvmtTime = [0 0.1 0.3 0.5 1];     % time after stimulus onset where movement is not counted (Before go sound)
%         data.params.earlyAbort = 30;
        data.params.reward = [6];
        data.params.rewardProb = [1];
        data.params.freeChoice = 0;     % percentage of free choices
        data.params.perRight = 0.5;       % percentage of right trials (out of all non-freechoice)
        ...
        data.params.perLaser = 0.40;    % percent laser trials
        data.params.noLaser = 1;        % number of no-laser trials at beginning
...
%         data.params.minGoDelay = 0;
% 
%         data.params.go_cue = 0;         % play go sound? (delay --> sound --> mvmt)
%         data.params.noMvmtTime = 0;     % time after stimulus onset where movement is not counted (Before go sound)
%         data.params.earlyAbort = 0;
        ...
        data.params.proOrienting = 0;
        data.params.flashStim = 0.4; %0.4
        data.params.earlyCue_sound = 0;
        
        data.params.contrast = 0.64;
        data.params.simultaneous = 1;
        data.params.opp_contrast = [0 0 0 0.16 0.32 0.48];
...
%         data.params.earlyAbort = 0;
      ...
      case 94
        ...
        data.params.responseTime = 15;   % maximum reaction time
        data.params.punishDelay = 0;
        
        data.params.freeBlank = 0;
        data.params.threshold = [15 15]; %in degrees, can have diff thresholds for left/right
        data.params.training = 1;
        data.params.trainingSide = [0 0]; % Set to [0 0] if training is on (1)
        data.params.reversalFreq = 0;
...
%         data.params.earlyAbort = 1;
        ...
        data.params.contrast = 1;
        data.params.simultaneous = 0;
        data.params.opp_contrast = [0]; %[0 0.16 0.32 0.48];
        ...
        data.params.laser_start = -0.31 ; % time after trial start
        data.params.perLaser = 0.4;    % percent laser trials
        data.params.noLaser = 0;        % number of no-laser trials at beginning
       
         data.params.laser_blank_only = 0.25;
...
%         data.params.earlyAbort = 1;
   ...
   case 3001
        ...
        data.params.antibiasConsecutive = 0; % must be consecutive
        data.params.firstBlockEqual = 1;
        
        %stim
        data.params.contrast = 0.001; % !
        data.params.freeBlank = 0;      % free choice is blank
    ...
    case 3002
        ...
        %timing
        data.params.responseTime = 6;   % maximum reaction time
        data.params.punishDelay = 0;
        data.params.actionValue = 1;    % reward amt is associated with left/right response (in blocks)
        data.params.blockSeq = randperm(2); % sequence of blocks
        
        % sound
        data.sound.noise_amp = 0.1;
  ...
  case {0301, 0302, 0303, 0304, 0305}
        data.params.lever = 1;
        data.params.reward = [6];
        data.params.rewardProb = [1];
        data.params.quitAfterMiss = 200;
        ...
        data.params.responseTime = 15;   % maximum reaction time
        data.params.lev_thresh = 0.40; % threshold difference between levers (percent of single threshold)
        data.params.punishDelay = 0;    % extra ITI on incorrect/timeout trials
        data.params.lev_pufftime = 0.25;    % airpuff time in sec
  ...
  case {0248,02,0250,0251,0286,0287, 0289,0290,0291,0292,0293,0294,7792,7797,5097,5098}% Chloe 2P4
        data.params.lever = 1;
        data.params.reward = [4];
        data.params.rewardProb = [1];
        data.params.quitAfterMiss = 200; 
        data.params.timeout = 200;
  ...
  case {0260,0261,0262,0263,0264,7796,7731,7793,7795,7791,7794} %Chloe 5th floor
        data.params.lever = 1;
        data.params.reward = [4];
        data.params.rewardProb = [1];
        data.params.quitAfterMiss = 200; 
    ...
    case 0
        data.params.freeChoice = 1;
        data.params.threshold = [450 450];
        data.params.responseTime = 5;
        data.params.punishDelay = 0;
...
fprintf('testing, waiting for touch...\n')
